...
public class OrderChefsFormController {
        ...
        try {
            ...
            for (int i = 0; i < orders.size(); i++) {
                ...
                for (int j = 0; j < orderItems.size(); j++) {
                    if (orderItems.get(j).getMenuItem().getMenuItemType() == MenuItemType.FOOD && !orderItems.get(j).getIsReady()) {
    ...
    public static void tblOrderMouseClicked(OrderChefsForm orderChefsForm) throws Exception {
        ...
        if (selectedRow != -1) {
            ...
            orderChefsForm.setOrder(order);
            orderChefsForm.getPnlOrderItems().setVisible(true);
            setOrderItemTableModel(order, orderChefsForm);
        }
    ...
    public static void setOrderItemTableModel(Order order, OrderChefsForm orderChefsForm) {
        JTable table = orderChefsForm.getTblOrderItems();
        List<OrderItem> filteredOrderItems = order.getOrderItems().stream()
                .filter(orderItem -> orderItem.getMenuItem().getMenuItemType() == MenuItemType.FOOD)
    ...
    public static void btnUpdateActionPerformed(OrderChefsForm orderChefsForm) {
        Order order = orderChefsForm.getOrder();
        ...
        int result = JOptionPane.showConfirmDialog(orderChefsForm, "Oznaci porudzbinu kao spremnu", "porudzbina broj: " + order.getOrderId(), JOptionPane.OK_CANCEL_OPTION);
        if (result == JOptionPane.OK_OPTION) {
            try {
                ...
                updateOrder(order);
                //proba
                orderChefsForm.getTblOrder().setSelectionBackground(Color.WHITE);

                orderChefsForm.setOrder(null);
        ...
        //orderChefsForm.getPnlOrderItems().setVisible(false);
    }
    ...
    public static void tblOrderItemsMouseClicked(OrderChefsForm orderChefsForm) {
        JTable table = orderChefsForm.getTblOrderItems();
