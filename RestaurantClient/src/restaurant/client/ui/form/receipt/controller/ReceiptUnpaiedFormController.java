...
public class ReceiptUnpaiedFormController {
    ...
    public static void setTableModel(JTable tblReceipt, JButton btnDetails, JButton btnPay) {
        List<Receipt> receipts = new ArrayList<>();
        User user = ApplicationSession.getInstance().getLoginUser();
        try {

            if (user.intRole() == 1) {
                receipts = getAllUnpaiedReceipts();
                tblReceipt.setModel(new ReceiptTableModel(receipts));
            } else {
                Employee employee = user.getEmployee();
                receipts = getAllUnpaiedReceiptsByEmployee(employee);
                tblReceipt.setModel(new ReceiptTableModel(receipts));
            }
